function magniglass(opt) {
    ...
    smaller.onmouseover = function (ev) {
        if (ev.target.tagName.toLowerCase() == 'img') {
            ...
            for (let i = 0; i < ev.target.parentNode.parentNode.children.length; i++) {
                ev.target.parentNode.parentNode.children[i].style.borderColor = '#eee';
            }

            ev.target.parentNode.style.borderColor = '#c01414';
        }
    }
    ...
    function move(speed) {//小图的运动
        var left = smaller.offsetLeft + speed;//ul这个盒子离左边的距离，往左滑，left为小于零的数
        if (left <= box.offsetWidth - smaller.offsetWidth + 10) {//临界值
            left = box.offsetWidth - smaller.offsetWidth + 10;
            // next.style.background = '#ccc';//（下一张不可以点击）
            // prev.style.background = '#777';//(上一张可以点击)
        } else if (left >= 0) {
            left = 0;
            // prev.style.background = '#ccc';//（上一张不可以点击）
            // next.style.background = '#777';//(下一张可以点击)
        }
